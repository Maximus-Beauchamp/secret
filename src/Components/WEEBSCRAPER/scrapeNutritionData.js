async function scrapeNutritionalInformation(page) {
    const nutritionalInfo = await page.evaluate(() => {
        const info = {
            ...
            allergens: [] // New array to store allergen information
        };
        ...
        // Extracting allergens from the specific tr with class "cbo_nn_LabelAllergens"
        const allergensRow = document.querySelector('.cbo_nn_LabelAllergens');
        if (allergensRow) {
            const allergens = allergensRow.innerText.trim().split(',').map(allergen => allergen.trim());
            info['allergens'] = allergens;
        } else {
            info['allergens'] = []; // Assign an empty array if no allergens are present
        }
